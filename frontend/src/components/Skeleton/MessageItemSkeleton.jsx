...
export default function MessageItemSkeleton({
    ...
    const nameHeight = "h-3.5 w-28";
    const line1 = compact ? "h-3 w-40" : "h-3 w-56";
    const line2 = compact ? "h-3 w-28" : "h-3 w-40";
    ...
    return (
        ...
        >
            ...
            <div className={`relative ${bubbleMaxWidth} group overflow-visible z-0`}>
                ...
                {showMedia && (
                    <div className={cn("mt-3 flex gap-2", isOwn ? "justify-end" : "justify-start")}>
                        <div className="h-16 w-24 rounded-md bg-muted animate-pulse" />
                        <div className="h-16 w-24 rounded-md bg-muted animate-pulse" />
                    </div>
                )}
