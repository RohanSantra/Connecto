...
export default function AuthEmailForm() {
  ...
  const handleSendOtp = async (e) => {
    ...
    if (!accepted)
      return setError("Please accept the Privacy Policy & Terms");
    ...
    try {
      ...
      await sendOtp(emailLocal.trim());

      toast.success("Verification code sent successfully.", {
        id: toastId,
      });

      localStorage.setItem("connecto_last_email", emailLocal.trim());
  ...
  const handleGoogleRedirect = () => {
    ...
    try {
      ...
      // 🔐 Get or create encryption keypair safely
      let publicKeyBase64;
      ...
      try {
        ...
        // corrupted storage — ignore and regenerate
      }
      ...
      <div className="max-w-md mx-auto w-full space-y-8 mt-20">
        ...
        <form onSubmit={handleSendOtp} className="space-y-5">
          ...
          <Button
            type="submit"
            className="w-full"
            disabled={!accepted || isOtpSending}
          >
            {isOtpSending ? (
              <>
          ...
          <Button
            ...
            disabled={isSending}
            className="w-full flex items-center justify-center gap-3 font-medium transition-all"
            aria-busy={isSending}
          >
            {isSending ? (
              <>
